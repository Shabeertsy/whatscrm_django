        ...
        {contacts.map(c => (
          <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group">
            ...
            <td className="px-4 py-3">
              {c.stage_name ? (
                <span className="flex items-center gap-1.5 px-2 py-1 rounded-full w-fit bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                  <span className="h-2 w-2 rounded-full flex-shrink-0" style={{ background: c.stage_color }} />
                  <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300">{c.stage_name}</span>
                </span>
              ) : (
                <span className="text-[10px] text-slate-400">No Stage</span>
              )}
            </td>
            <td className="px-4 py-3">
